...
<display:table name="sensorList" class="list" cellspacing="1" requestURI="" id="row" export="false" pagesize="${GLOBAL_pagesize}" sort="list">
    ...
    <display:column property="deviceProperty.name" sortable="true" titleKey="deviceProperty.name" style="text-align:left" />
    
    <display:column property="mydvr.name" sortable="true" titleKey="myDVR.name" style="text-align:left" />
    <display:column property="channel" sortable="true" titleKey="myDVR.channel" style="text-align:left" />
    
    
    <display:column titleKey="common.action">
        <ww:if test="recordEditable == true">
            <a href="editSensor.html?sensor.id=<c:out value="${row.id}"/>"><fmt:message key="common.edit"/></a>&nbsp;&nbsp;
            <a href="#" onClick="javascript:return myConfirm('<fmt:message key="common.confirmDelete1"/><c:out value="${row.name}" /><fmt:message key="common.confirmDelete2"/>', 'form1', 'sensor.id', '${row.id}')"><fmt:message key="common.delete"/></a>
        </ww:if>
        <ww:else>
