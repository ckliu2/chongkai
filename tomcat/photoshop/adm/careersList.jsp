...
<table border="0" cellspacing="1" >
      ...
      <tr class="cActionButtonLine">
        ...
        <ww:if test="recordEditable == true">
            <button onclick="location.href='editCareers.html'" class="cButton"><fmt:message key="common.new"/></button>
            <ww:if test="functionOpenPeriodMessage != null">
                <span class="cLabel2">(<fmt:message key="common.function.openPeriod"/>: <ww:property value="functionOpenPeriodMessage"/>)</span>
            </ww:if>
        </ww:if>
        <ww:else>
            <button onclick="location.href='editCareers.html'" DISABLED class="cButton"><fmt:message key="common.new"/></button><span class="cLabel2">(<fmt:message key="common.function.openPeriodExceed"/> )</span>
        </ww:else>
        </td>
      </tr>
      <tr>
...
<display:table name="careersList" class="list" cellspacing="1" requestURI="" id="row" export="false" pagesize="${GLOBAL_pagesize}" sort="list">
    ...
    <display:column titleKey="common.action">
        <ww:if test="recordEditable == true">
            <a href="editCareers.html?careers.id=<c:out value="${row.id}"/>"><fmt:message key="common.edit"/></a>&nbsp;&nbsp;
        ...
        <ww:else>
           <a href="editCareers.html?careers.id=<c:out value="${row.id}"/>"><fmt:message key="common.view"/></a>&nbsp;&nbsp;
        </ww:else>
    </display:column>
...
<form name="form1" action="saveCareers.html">
...
<script>
  ...
  function allow(count,id){  	  
  	  ...
  	  sendRequest("careersAllow.html?careers.id="+id+"&careers.allow="+s.checked);  	 
  }
  ...
  var tall = eval('document.all.selectall') ;   
	...
	sendRequest("careersAllowAll.html?idx="+idx+"&careers.allow="+tall.checked);
	//alert(idx);	  
