...
<div class="box3">
    ...
    <display:table name="bulletinList" style="width: 100%" class="tb4" cellspacing="1" requestURI="" id="tb4" export="false" pagesize="${GLOBAL_pagesize}" sort="list" >
      ...
      <c:if test="${bulletinTypeId==21}">    
        <display:column titleKey="bulletin.subject" headerClass="td_hd" style="text-align:left;width: 30%"  sortable="false" >            
           <a href="<%=com.common.util.Util.IMAGE_CONTEXT_ROOT_PATH%>/upload/bulletin/${tb4.id}/${tb4.file.fileName}" class="link2" target="_blank">${tb4.subject}</a>
       </display:column>
      </c:if> 
       ...
       <display:column titleKey="bulletin.msg1" headerClass="td_hd" style="text-align:left;width: 12%" >        
              <c:choose>
               <c:when test="${tb4.file != null}"> 
                   <a href="${ctx}/<fmt:message key="bulletin.uploadFile.relativeUrl"/>/${tb4.id}/${tb4.file.fileName}" class="pdf" target="_blank"><b>下載大綱</b></a>
               </c:when>   
               <c:otherwise>  
        ...
        <display:column titleKey="bulletin.online" headerClass="td_hd" style="text-align:left;width: 12%"> 
            <a href="bulletinDetail.do?bulletinId=${tb4.id}" class="video"><b>線上收看</b></a>
        ...
        <display:column titleKey="bulletin.online1" headerClass="td_hd" style="text-align:left;width: 12%"> 
            <c:choose>
               <c:when test="${tb4.voice != null}"> 
                   <a href="${ctx}/<fmt:message key="bulletin.uploadFile.relativeUrl"/>/${tb4.id}/${tb4.voice.fileName}" class="voice" target="_blank"><b>線上收聽</b></a>
               </c:when>   
               <c:otherwise>  
