...
<display:table name="memberList" class="list" defaultsort="3"  cellspacing="1" requestURI="" id="row" export="false" pagesize="20" sort="list"  >
    ...
    <display:column titleKey="common.action">
      ...
      <display:column titleKey="member.name" style="text-align:left" sortable="true">     
        <a href="editMember.do?member.id=<c:out value="${row.id}"/>"><c:out value="${row.name}" /></a>
     </display:column> 
     
      <display:column titleKey="unit.name" style="text-align:left" sortable="true">        
          <c:out value="${row.units}" /> 
     </display:column>           

     
    <display:column titleKey="team.name" style="text-align:left" sortable="true">        
          <c:out value="${row.teams}" /> 
    </display:column>  
    
    <display:column titleKey="grade.name" style="text-align:left" sortable="true">        
          <c:out value="${row.grades}" /> 
    </display:column>    
    

    
    <display:column titleKey="group.groupNameCh" style="text-align:left" sortable="true">        
          <c:out value="${row.groups}" /> 
    </display:column> 
    
    <display:column titleKey="industry.name" style="text-align:left" sortable="true">        
          <c:out value="${row.industrys}" /> 
    </display:column>
