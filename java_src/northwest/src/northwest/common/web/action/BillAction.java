...
public class BillAction extends CommonActionSupport {
    ...
    private String fileImageDiagramContentType, fileImageDiagramFileName,
	    removeImageDiagram;
    private java.io.File fileCheckDiagram;
    private String fileCheckDiagramContentType, fileCheckDiagramFileName,
	    removeCheckDiagram;
    private java.io.File fileCompletionphoto1;
    private BillSchedule billSchedule;
    private CustomerDetail customerDetail;
    private String completionTime, check2, check5, productId, outputTime,
	    productionTime, deliveryTime, arrival, uploadDrawingsTime,
	    customerId = "";
	...
	try {
	    billSchedule = getGenericManager().getBillScheduleById(
		    billSchedule.getId());
	} catch (Exception e) {
	...
	System.out.println("checkBillFiles billFiles.id=" + billFiles.getId()
	...
	System.out.println("edit1() billFiles.id=" + billFiles.getId()
		+ "--billFiles.getBill().getId()="
		+ billFiles.getBill().getId());
	...
	System.out.println("editDrawing() billSchedule.getBillDetailId()="
		+ billSchedule.getBillDetailId());
	billSchedule = getGenericManager().getBillScheduleById(
		billSchedule.getBillDetailId());
	...
	checkBillFinish(); // 判斷是否結案
	saveFileToLocal(fileDoc1FileName, fileDoc1,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	saveFileToLocal(fileDoc2FileName, fileDoc2,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	saveFileToLocal(fileDoc3FileName, fileDoc3,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	saveFileToLocal(fileDoc4FileName, fileDoc4,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	saveFileToLocal(fileDoc5FileName, fileDoc5,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	saveFileToLocal(fileDoc6FileName, fileDoc6,
		getTextWithArgs("billFiles.uploadDoc.dir"), billFiles.getId());
	...
	if (getRemoveDoc1() != null && getRemoveDoc1().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc1Id(), billFiles.getDoc1FileName());
	} else {
	    if (billFiles.getDoc1Id() != null)
		billFiles.setDoc1(getGenericManager().getUploadedFileById(
			billFiles.getDoc1Id()));
	    else
		billFiles.setDoc1(computeUploadedFile(fileDoc1FileName,
			fileDoc1));
	}
	...
	if (getRemoveDoc2() != null && getRemoveDoc2().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc2Id(), billFiles.getDoc2FileName());
	} else {
	    if (billFiles.getDoc2Id() != null)
		billFiles.setDoc2(getGenericManager().getUploadedFileById(
			billFiles.getDoc2Id()));
	    else
		billFiles.setDoc2(computeUploadedFile(fileDoc2FileName,
			fileDoc2));
	}
	...
	if (getRemoveDoc3() != null && getRemoveDoc3().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc3Id(), billFiles.getDoc3FileName());
	} else {
	    if (billFiles.getDoc3Id() != null)
		billFiles.setDoc3(getGenericManager().getUploadedFileById(
			billFiles.getDoc3Id()));
	    else
		billFiles.setDoc3(computeUploadedFile(fileDoc3FileName,
			fileDoc3));
	}
	...
	if (getRemoveDoc4() != null && getRemoveDoc4().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc4Id(), billFiles.getDoc4FileName());
	} else {
	    if (billFiles.getDoc4Id() != null)
		billFiles.setDoc4(getGenericManager().getUploadedFileById(
			billFiles.getDoc4Id()));
	    else
		billFiles.setDoc4(computeUploadedFile(fileDoc4FileName,
			fileDoc4));
	}
	...
	if (getRemoveDoc5() != null && getRemoveDoc5().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc5Id(), billFiles.getDoc5FileName());
	} else {
	    if (billFiles.getDoc5Id() != null)
		billFiles.setDoc5(getGenericManager().getUploadedFileById(
			billFiles.getDoc5Id()));
	    else
		billFiles.setDoc5(computeUploadedFile(fileDoc5FileName,
			fileDoc5));
	}
	...
	if (getRemoveDoc6() != null && getRemoveDoc6().length() > 0) {
	    ...
	    removeUploadedFile(getTextWithArgs("billFiles.uploadDoc.dir"),
		    billFiles.getDoc6Id(), billFiles.getDoc6FileName());
	} else {
	    if (billFiles.getDoc6Id() != null)
		billFiles.setDoc6(getGenericManager().getUploadedFileById(
			billFiles.getDoc6Id()));
	    else
		billFiles.setDoc6(computeUploadedFile(fileDoc6FileName,
			fileDoc6));
	}
	...
	if (stepIds != null) {
		...
		case 1:
		    ...
		    if (getGenericManager().isSettingPermissions(member,
			    getGenericManager().getGroupById(groupId)) == false
			    && obj.getStep1() == false) {
			obj.setStep1(true);
			obj.setCheck1(now);
	...
	// remove
	List<BillAuthority> b = getGenericManager().getBillAuthorityListByBill(
		bill);
	...
	// add
	Set groups = getGenericManager().getGroupListByIds(
		billGroupAuthorityIds);
	if (groups != null) {
	...
	// remove
	List<BillDetail> d1 = getGenericManager().getBillDetailListByBillNo(
		bill.getId());
	for (int k = 0; k < d1.size(); k++) {
	    BillDetail obj = (BillDetail) d1.get(k);
	    BillSchedule s = getGenericManager().getBillScheduleByBillDetail(
		    obj);
	    s.setStep1(false);
	    s.setStep2(false);
	    s.setStep3(false);
	...
	if (groups != null) {
		...
		List<BillDetail> d = getGenericManager()
			.getBillDetailListByBillNo(bill.getId());
		for (int j = 0; j < d.size(); j++) {
		    BillDetail obj = (BillDetail) d.get(j);
		    BillSchedule s = getGenericManager()
			    .getBillScheduleByBillDetail(obj);
		    switch (step) {
		    case 1:
			s.setStep1(true);
			break;
	...
	System.out.println("startDate=" + startDate + "--endDate=" + endDate
		+ "--state=" + state + "--billNo=" + billNo);

	List<Bill> al = getGenericManager().getBillListByCondition(customer,
		billNo.trim(), state, startDate, endDate, pageSize, pageIndex);
	int resultSize = getGenericManager().getBillListByCondition(customer,
		billNo.trim(), state, startDate, endDate);
	...
	List<BillAuthority> al = getGenericManager()
		.getBillAuthorityListByBill(
			billSchedule.getBillDetail().getBill());
	...
	List<BillDetail> al = getGenericManager().getBillDetailListByBillNo(
		BillNo);
	...
	for (int i = 0; i < al.size(); i++) {
	    ...
	    System.out.println("s.getId()=" + s.getId() + "---result=" + result
		    + "--result1=" + result1 + "-result2=" + result2 + "--g="
		    + g.getGroupName() + "--firstGroup="
		    + firstGroup.getGroupName() + "--group="
		    + group.getGroupName());
    ...
    public List<BillDetail> getSelectedBillDetailIds(String BillNo,
	    String groupId) {
	ArrayList<BillDetail> ls = new ArrayList<BillDetail>();
	List<BillDetail> al = getGenericManager().getBillDetailListByBillNo(
		BillNo);
	...
	if (w != null) {
	    ...
	    al = getGenericManager().getBillDetailListByBillNo(bill, pageSize,
		    pageIndex, w);
	    resultSize = getGenericManager().getBillDetailListByBillNoWorker(
		    bill, w);
	} else {
	    System.out.println("w==null");
	    al = getGenericManager().getBillDetailListByBillNo(bill, pageSize,
		    pageIndex);
	    resultSize = getGenericManager().getBillDetailListByBillNo(BillNo)
		    .size();
	}
	...
	if (fromRow1 != 0 && fromRow2 != 0) {
	    System.out.println("BillAction fromRow1=" + fromRow1
		    + "---fromRow2=" + fromRow2);
	    al = getGenericManager().getBillDetailListByBillNo(BillNo,
		    fromRow1, fromRow2);
	}
	...
	List<BillDetail> obj = (List<BillDetail>) getSession().getAttribute(
		"products");
	return obj;
    }
	...
	List<BillAuthority> ba = getGenericManager()
		.getBillAuthorityListByBill(bill);
	...
	List<BillAuthority> objList = getGenericManager()
		.getBillAuthorityListByBill(bill);
	for (int i = 0; i < objList.size(); i++) {
	    BillAuthority obj = (BillAuthority) objList.get(i);
	    int complete = getGenericManager().getAllCompleteStatusByBill(
		    obj.getBill(), obj.getGroup(), true);
	    int uncomplete = getGenericManager().getAllCompleteStatusByBill(
		    obj.getBill(), obj.getGroup(), false);
	    obj.setComplete(complete);
	    obj.setUncomplete(uncomplete);
	    // System.out.println(obj.getGroup().getGroupNameEn()+"  complete="+complete+"---uncomplete="+uncomplete);
	}
    ...
    public boolean getRecordEditable(String billScheduleId, String gid,
	    boolean isPermissions) {

	boolean result;
	BillSchedule billSchedule = getGenericManager().getBillScheduleById(
		billScheduleId);
	Group group = getGenericManager().getGroupById(Long.parseLong(gid));
	Group previousGroup = getGenericManager().previousStep(billSchedule,
		group);

	Product product = billSchedule.getBillDetail().getProduct();
	boolean s = billSchedule.getBillDetail().getProduct().getProductClass()
		.getExpectionFlows().contains(previousGroup);

	boolean b = getGenericManager().isEnabledEdit1(billSchedule,
		previousGroup);
	if (s == true) {
	    result = false;
	} else {
	...
	System.out.println("group=" + group.getGroupName()
		+ "----previousGroup=" + previousGroup.getGroupName() + "--b="
		+ b + "---result=" + result);
	return result;
    }
	...
	try {
	    Group group = getGenericManager().getGroupById(Long.parseLong(gid));
	    BillSchedule billSchedule = getGenericManager()
		    .getBillScheduleById(billScheduleId);
	    // System.out.println("getPass ProductClassID="+billSchedule.getBillDetail().getProduct().getProductClass().getId()+"--groupid="+group.getId());
	    List ls = billSchedule.getBillDetail().getProduct()
		    .getProductClass().getExpectionFlows();
	    if (ls != null) {
	...
	System.out.println("saveBillOutputDevice()...bill.getId()="
		+ bill.getId() + "---" + billSchedule.getOutputDevice());
	...
	for (int i = 0; i < billDetailIds.length; i++) {
	    ...
	    s.setOutputEquipment(getGenericManager().getOutputEquipmentById(
		    billSchedule.getOutputEquipmentId()));
	    getGenericManager().saveBillSchedule(s);
	}
	appendXworkParam("bill.id=" + bill.getId());
	...
	for (int i = 0; i < billDetailIds.length; i++) {
	    String id = billDetailIds[i];
	    BillDetail obj = getGenericManager().getBillDetailById(id);
	    BillSchedule b = getGenericManager().getBillScheduleByBillDetail(
		    obj);
	    b.setStep1(true);
	    b.setCheck1(Tools.convertToDateTime(completionTime));
	    b.setChecker1(s.getMember());
	    b.setFileName(billSchedule.getFileName());
	    b.setCustomerScreen(getGenericManager().getCustomerScreenById(
		    billSchedule.getCustomerScreenId()));
	    b.setWorker(getGenericManager().getMemberById(
		    billSchedule.getWorkerId()));
	...
	if (!bill.getId().equals("")) {
	    ...
	    saveFileToLocal1(fileImageDiagramFileName, fileImageDiagram,
		    ...
		    bill.getId() + "_ImageDiagram");
	    saveFileToLocal1(fileCheckDiagramFileName, fileCheckDiagram,
		    getTextWithArgs("billSchedule.uploadCheckDiagram.dir"),
		    bill.getId() + "_CheckDiagram");
		...
		BillSchedule billSchedule = getGenericManager()
			.getBillScheduleByBillDetail(obj);
		billSchedule.setImageDiagram(computeUploadedFile(
			fileImageDiagramFileName, fileImageDiagram));
		billSchedule.setCheckDiagram(computeUploadedFile(
			fileCheckDiagramFileName, fileCheckDiagram));
		...
		if (j == 0) {
		    saveFileToLocal1(
			    fileImageDiagramFileName,
			    fileImageDiagram,
			    getTextWithArgs("billSchedule.uploadImageDiagram.dir"),
			    ls.get(j) + "_ImageDiagram");
		    saveFileToLocal1(
			    fileCheckDiagramFileName,
			    fileCheckDiagram,
			    getTextWithArgs("billSchedule.uploadCheckDiagram.dir"),
			    ls.get(j) + "_CheckDiagram");
		} else {
		    try {
			File imageDiagram = new File(
				getText("billSchedule.uploadImageDiagram.filepath")
					+ "//"
					+ ls.get(0)
					+ "_ImageDiagram//"
					+ fileImageDiagramFileName);
			File imageDiagramTarget = new File(
				getText("billSchedule.uploadImageDiagram.filepath")
					+ "//" + ls.get(j) + "_ImageDiagram//",
				fileImageDiagramFileName);
			fileCopy(
			...
			File checkDiagram = new File(
				getText("billSchedule.uploadCheckDiagram.filepath")
					+ "//"
					+ ls.get(0)
					+ "_CheckDiagram//"
					+ fileCheckDiagramFileName);
			File checkDiagramTarget = new File(
				getText("billSchedule.uploadCheckDiagram.filepath")
					+ "//" + ls.get(j) + "_CheckDiagram//",
				fileCheckDiagramFileName);
			fileCopy(
	...
	for (int i = 0; i < billDetailIds.length; i++) {
	    ...
	    b.setOutputEquipment(getGenericManager().getOutputEquipmentById(
		    billSchedule.getOutputEquipmentId()));
	    b.setStep7(true);
	...
	for (int i = 0; i < billDetailIds.length; i++) {
	    ...
	    BillSchedule b = getGenericManager().getBillScheduleByBillDetail(
		    obj);
	    b.setIsSend(getGenericManager().getAppPropertyById(
		    billSchedule.getIsSendId()));
	    b.setFreight(getGenericManager().getFreightById(
		    billSchedule.getFreightId()));
	...
	saveFileToLocal1(fileDrawingFileName, fileDrawing,
	...
	if (billDetailIds != null) {
		...
		String id = billDetailIds[i];
		BillDetail obj = getGenericManager().getBillDetailById(id);
		BillSchedule b = getGenericManager()
			.getBillScheduleByBillDetail(obj);
		b.setUploadDrawingsTime(Tools
			.convertToDateTime(uploadDrawingsTime));
		...
		if (drawingsFilesIds != null) {
			...
			UploadedFile file = getGenericManager()
				.getUploadedFileById(drawingsFilesIds[j]);
			al.add(file);
		    }
		}
		if (fileDrawing != null) {
		    UploadedFile u = computeUploadedFile(fileDrawingFileName,
			    fileDrawing);
	...
	System.out.println("saveBillDeliveryTimeTime()...bill.getId()="
		+ bill.getId());
	...
	return getGenericManager()
		.getCustomerDetailById(customerDetail.getId());
    }

    public boolean isEnabledEdit(String billScheduleId, String groupKey) {
	System.out.println("isEnabledEdit=  billScheduleId==" + billScheduleId
		+ "  groupKey=" + groupKey);
	BillSchedule b = getGenericManager()
		.getBillScheduleById(billScheduleId);
	...
	int pageSize = getPageSize();
	// System.out.println("getBillScheduleListByCondition pageIndex="+pageIndex+"----pageSize="+pageSize);
	List<Meter> alist;
	alist = getGenericManager().getMeterListByCondition(
		Long.parseLong(group.getGroupNameEn()), product, result,
		pageSize, pageIndex);
	int resultSize = getGenericManager().getMeterListByCondition(
		Long.parseLong(group.getGroupNameEn()), product, result).size();
	getSession().setAttribute("RESULT_SIZE", new Integer(resultSize));

	List<BillDetail> ls = new ArrayList<BillDetail>();
	for (int i = 0; i < alist.size(); i++) {
	    BillDetail b = alist.get(i).getBilldetail();
	    System.out.println("getBillScheduleListByCondition products="
		    + b.getId());
	    ls.add(b);
	}
	...
	return getGenericManager().getProductListByCondition(
	...
	System.out.println("bill.id" + bill.getId() + " id=" + id + "--" + id
	...
	if (group == null) {
	    ...
	    return getGenericManager().getProcessStateList(billNo, group,
		    status, 1, billSchedule.getWorkerId());
	}
    }
	...
	String sql = " SELECT *  FROM WorkFlowStatus WHERE billNo LIKE'%"
		+ billNo + "%' ORDER BY billNo DESC ";
	System.out.println(sql);
	getGenericManager().exportExcel(sql, "report1.xls");
    }

    public void exportExcel2() {
	String sql = " SELECT *  FROM StatisticsReport WHERE billNo LIKE'%"
		+ billNo + "%' ORDER by BillNO,SN ASC ";
	System.out.println(sql);
