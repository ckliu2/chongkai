...
<display:table name="courseMemberList" class="list" cellspacing="1" requestURI="" id="row" export="false" pagesize="${GLOBAL_pagesize}" sort="list">
    ...
    <display:column property="complete" sortable="true" titleKey="courseMember.complete" style="text-align:left" />
    ...
    <display:column property="course.caption_" sortable="true" titleKey="courseMember.course" style="text-align:left"/>
    <display:column property="member.caption_" sortable="true" titleKey="courseMember.member" style="text-align:left"/>
    <display:column titleKey="common.action">
        <ww:if test="recordEditable == true">
            <a href="editCourseMember.html?courseMember.id=<c:out value="${row.id}"/>"><fmt:message key="common.edit"/></a>&nbsp;&nbsp;
            <a href="#" onClick="javascript:return myConfirm('<fmt:message key="common.confirmDelete1"/><c:out value="${row.caption_}" /><fmt:message key="common.confirmDelete2"/>', 'form1', 'courseMember.id', '${row.id}')"><fmt:message key="common.delete"/></a>
        </ww:if>
        <ww:else>
...
<form name="form1" action="saveCourseMember.html">
    <input type="hidden" name="delete" value="yes">
    <input type="hidden" name="courseMember.id" value="">
    <select name="selectedCourseMemberIds" multiple="true" style="display:none"></select>
</form>
