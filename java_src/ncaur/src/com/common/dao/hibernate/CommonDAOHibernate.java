...
public class CommonDAOHibernate extends BaseDAOHibernate implements CommonDAO {
	...
	public void removeMember(Member val) {
		...
		try {
			...
			String sql = "delete WEB_AgreeTerms where createdUser='" + val.getId() + "' ";
			Stmt.execute(sql);
			conn.close();
		} catch (Exception e) {
	...
	public void removeMember(Long id) {
		...
		try {
			Connection conn = getHibernateSession().connection();
			Statement Stmt = conn.createStatement();
			String sql = "delete WEB_AgreeTerms where createdUser='" + id + "' ";
			Stmt.execute(sql);
			conn.close();
		} catch (Exception e) {
			System.out.println(e.toString());
	...
	public List<Member> findAllMember(String keyword) {
		...
		if (keyword != null) {
			c.add(Restrictions.or(Restrictions.like("cname", "%" + keyword + "%"), Restrictions.like("ename", "%" + keyword + "%")));
		}
		c.addOrder(Order.asc("cname"));
		return c.list();
	}
	...
	public List<Member> getMemberListById(Long[] ids) {
		...
		if (ids != null) {
			for (int i = 0; i < ids.length; i++) {
				...
				System.out.println("getMemberListByIds 加入Member name=" + m.getCname());
				al.add(m);
			}
		}
		return al;
	...
	public Member findMemberByEmailAndCellphone(String email, String cellphone) {
		Member user = null;
	...
	public Member getMemberByIdNoAndBirthday(String idNo, String brithday) {
		...
		try {
			Criteria c = getHibernateSession().createCriteria(Member.class);
			c.add(Expression.eq("idNo", idNo));
			// c.add(Expression.eq("brithday",
			// Tools.convertToDate1("19760528")));
