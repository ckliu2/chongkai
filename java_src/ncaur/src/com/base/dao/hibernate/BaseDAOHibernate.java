...
public class BaseDAOHibernate extends HibernateDaoSupport implements BaseDAO {
	...
	public List<AppProperty> findAppPropertyListBySubKind(String subkind) {
		Criteria c = getHibernateSession().createCriteria(AppProperty.class).add(Expression.eq("subKind", subkind));
		return c.list();
	}
	...
	public void sendMail(String mail, String subject, String body) {
		...
		try {
			...
			String sql = "dbo.sendMail '" + mail + "','" + subject + "','" + body + "'";
			System.out.println("sql=" + sql);
			Stmt.execute(sql);
			conn.close();
		} catch (Exception e) {
			System.out.println("sendMail error" + e.toString());
		}
	}
