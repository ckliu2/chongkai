...
public abstract class BaseActionSupport extends ActionSupport
{
    ...
    public SessionUser getAlumniUser() 
    {
        HttpSession ses = getSession();
        SessionUser user = (SessionUser) ses.getAttribute("alumni");
        if (user == null)
            throw new SessionTimeoutException(getText("common.sessionTimeout"));
    ...
    {
        ActionContext ctx = ActionContext.getContext();
        HttpServletResponse response = (HttpServletResponse) ctx.get(HTTP_RESPONSE);
        return response;
    }
