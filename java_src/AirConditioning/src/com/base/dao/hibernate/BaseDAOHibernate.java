...
public class BaseDAOHibernate extends HibernateDaoSupport implements BaseDAO
{
    ...
    public List<Function> findFunctionListBySessionUser(SessionUser su)
    {
        ...
        for (int i=0; i < lst.size(); i++) {
        	...
        	//System.out.println("2可以授權使用的Menu>>" +ff.getCategory().getName()+"/Item="+ ff.getFunName() );
        	
        	Iterator group = su.getRole().iterator();    	
    		while (group.hasNext()) {
                 Group g= (Group) group.next();    
                 
                 //System.out.println("1可以授權使用的Menu>>" +ff.getCategory().getName()+"/Item="+ ff.getFunName() + ">>" + g.getGroupNameCh());
                 
                if (ff.containGroup(g) == true) {
                     
                    if(result.contains(ff)==false){ 
                      //System.out.println("可以授權使用的Menu>>" +ff.getCategory().getName()+"/Item="+ ff.getFunName() + ">>" + g.getGroupNameCh());
                	  result.add(ff);
                    }
                     //continue;
            	}
    ...
    /*public List<Function> findFunctionListBySessionUser(SessionUser su)
    {
        ...
        for (int i=0; i < lst.size(); i++) {
    		...
    		while (group.hasNext()) {
                ...
                if (ff.containGroup(g) == true) {
                    //System.out.println("可以授權使用的MenuItem>" + ff.getFunName() + ">>" + ff.getCategory().getSeqNo());
                    result.add(ff);
                    continue;
            	}
            }
